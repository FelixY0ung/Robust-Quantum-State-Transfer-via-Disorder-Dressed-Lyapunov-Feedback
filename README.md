...
python3 code/adaptive_operator_net_terminal_value_horizon.py
python3 code/operator_net_training_strategy_comparison.py
python3 code/selected_operator_net_strategy_gradient_certificate.py
python3 code/selected_operator_net_strategy_gradient_certificate.py --certificate-points 21 --output-prefix selected_operator_net_strategy_gradient_certificate_p21
...
python3 code/fixed_depth_terminal_value_margin_audit.py --controllers terminal_value_shifted --depths 1 --stride 10 --batch-size 512 --terminal-weights 0,1 --control-stage-weights 0,0.0001,0.0005,0.001,0.002 --output-prefix fixed_depth_terminal_value_stage_weight_sweep
...
The operator-net gradient certificate audit propagates first-order terminal-infidelity sensitivities on the same 2145-point Pauli-ball net and combines the maximum audited gradient with the analytic Hessian envelope `4T^2 = 256`. The maximum gradient norms are `0.172701/0.0672411` for Z/H, giving certified sensitivity constants `3.12873/3.02327` and continuous-ball fidelity lower bounds `0.954975/0.961207`. This is a sharper deterministic sensitivity certificate than the global `2T h` bound, while still remaining conservative.

The selected operator-net strategy certificate keeps the best deterministic strategy-comparison rows fixed: Z uses a 547-point hard-augmented mean-plus-worst design and H uses full-2145 max-net scoring. A denser 21-point-per-axis certificate run evaluates 9181 Pauli-ball points with covering radius `h = 0.0069282`; worst net fidelity is `0.996298/0.996966` for Z/H, maximum gradient norms are `0.0741829/0.063548`, and the certified continuous-ball fidelity lower bounds improve to `0.983497/0.984238`. This is still finite-run strategy-selection evidence, not a fixed-depth all-time value theorem.
...
The fixed-depth terminal-value stage-weight sweep tests whether small quadratic fallback-control costs repair the same unsupported condition. With terminal weights `0` and `1` and control-stage weights through `0.002`, H remains zero-positive outside residual; the least negative H row is unchanged at minimum outside margin `-0.000566451`. This keeps the boundary conclusion unchanged: light stage-cost shaping is not the missing fixed-depth design.
...
- `results/operator_net_training_strategy_comparison_summary.md`
- `results/selected_operator_net_strategy_gradient_certificate_summary.md`
- `results/selected_operator_net_strategy_gradient_certificate_p21_summary.md`
...
- `results/fixed_depth_terminal_value_stage_weight_sweep_summary.md`
