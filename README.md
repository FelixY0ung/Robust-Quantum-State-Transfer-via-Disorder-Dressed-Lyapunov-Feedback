...
The open-leakage Pareto audit reuses the same combined-noise rows and reports the mean final fidelity versus mean maximum leakage tradeoff. The direct adjoint is the lowest-leakage Pareto point, the Pareto-refinement sweep fills the low-to-mid leakage no-reference front, the integrated direct sweep adds a higher-fidelity no-reference Pareto point, the target-biased direct row is the strongest GRAPE-free Pareto point before leakage grows sharply, and the two-stage direct row gives the best GRAPE-free worst held-out seed but is dominated in mean fidelity/leakage by the reference-assisted horizon and leakage-GRAPE. The generated files are `results/open_leakage_integrated_sweep_results.csv`, `results/open_leakage_integrated_sweep_summary.md`, `results/figures/open_leakage_integrated_sweep.*`, `results/open_leakage_pareto_refinement_results.csv`, `results/open_leakage_pareto_refinement_summary.md`, `results/figures/open_leakage_pareto_refinement.*`, `results/open_leakage_pareto_audit_results.csv`, `results/open_leakage_pareto_audit_summary.md`, and `results/figures/open_leakage_pareto_audit.*`.
