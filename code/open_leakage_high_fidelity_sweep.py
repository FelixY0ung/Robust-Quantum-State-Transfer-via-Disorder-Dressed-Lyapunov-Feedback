"""High-fidelity no-reference continuation audit for open leakage dynamics.

The existing continuation sweep identifies a useful no-reference frontier for
the five-level leakage-plus-Lindblad setting, but its highest-fidelity row still
uses a final leakage weight of 0.8.  This script tests whether a more aggressive
terminal-target push can improve the GRAPE-free upper end of that frontier
without using a terminal GRAPE reference.
"""
...
VARIANTS = (
    Variant(
        "hf_leak06",
        "target push leak 0.6",
        (
            StageSpec(1.0, 0.8, 0.02, 6),
            StageSpec(1.0, 0.6, 0.015, 6),
        ),
    ),
    Variant(
        "hf_leak05",
        "target push leak 0.5",
        (
            StageSpec(1.0, 0.8, 0.02, 6),
    ...
    Variant(
        "hf_leak04",
        "target push leak 0.4",
        (
            StageSpec(1.0, 0.8, 0.02, 6),
            StageSpec(1.0, 0.4, 0.015, 6),
        ),
    ),
    Variant(
        "hf_leak03",
        "target push leak 0.3",
        (
            StageSpec(1.0, 0.8, 0.02, 6),
            StageSpec(1.0, 0.3, 0.015, 6),
        ),
    ),
    Variant(
        "hf_leak05_worst05",
        "target push leak 0.5, worst 0.5",
        (
    ...
    Variant(
        "hf_leak04_wide",
        "target push leak 0.4, wider final trust",
        (
            StageSpec(1.0, 0.8, 0.02, 6),
            StageSpec(1.0, 0.4, 0.02, 6),
        ),
    ),
)
...
def annotate_rows(
    ...
    for row in rows:
        item = dict(row)
        item.update({"hf_label": label, "stage_description": stage_text})
...
def plot_results(rows: list[dict[str, float | int | str]]) -> None:
    ...
    for controller, (mean_fid, worst_fid, leak) in new_points.items():
        if controller in {"hf_path_seed", "hf_low_leak_seed"}:
            continue
        ax.scatter(leak, mean_fid, marker="D", s=44, color="#d62728")
        ax.annotate(
            controller.replace("hf_", ""),
            (leak, mean_fid),
            xytext=(4, 3),
            textcoords="offset points",
            fontsize=7,
        )
        ax.scatter(leak, worst_fid, marker="x", s=28, color="#d62728")
    ax.set_xlabel("Mean maximum leakage")
    ax.set_ylabel("Held-out final fidelity")
    ax.set_title("High-fidelity no-reference open-leakage sweep")
    ax.set_xlim(0.0, 0.09)
    ax.set_ylim(0.50, 0.97)
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(figure_path("open_leakage_high_fidelity_sweep.pdf"))
    fig.savefig(figure_path("open_leakage_high_fidelity_sweep.png"), dpi=220)
    plt.close(fig)


def write_outputs(rows: list[dict[str, float | int | str]]) -> None:
    result_file = result_path("open_leakage_high_fidelity_sweep_results.csv")
    ...
    summary_file = result_path("open_leakage_high_fidelity_sweep_summary.md")
    with summary_file.open("w", encoding="utf-8") as f:
        f.write("# High-Fidelity Open-Leakage Continuation Sweep\n\n")
        f.write(
            "No-reference target-push continuations trained through the five-level "
    ...
    print(f"wrote figure to {figure_path('open_leakage_high_fidelity_sweep.pdf')}")
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.only:
        ...
        if not variants:
            raise ValueError(f"no high-fidelity variants matched --only={args.only!r}")
