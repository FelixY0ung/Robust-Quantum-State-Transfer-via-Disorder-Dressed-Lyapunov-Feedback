...
1. Syntax/reproducibility checks
   ...
   - Confirmed output sizes:
    ...
    - `results/ensemble_lyapunov_results.csv`: 100 rows, Z/H tasks, seeds
     ...
     - `results/open_leakage_pareto_refinement_results.csv`: 1000 rows, 5
       controllers, 4 noise cases, strength `0.03`, seeds `10..59`.
     - `results/open_leakage_continuation_sweep_results.csv`: 1000 rows, 5
       controllers, 4 noise cases, strength `0.03`, seeds `10..59`.
     - `results/open_leakage_pareto_audit_results.csv`: 12 rows after adding
       the Pareto-refinement and continuation points.
   ...
   - Ran `python3 code/reproducibility_manifest.py`; the manifest recorded
     SHA-256 hashes, file sizes, CSV row counts, and CSV columns for 45 code
     files and 145 result artifacts. Key row-count checks include
     ...
     `results/open_leakage_continuation_sweep_results.csv` with 1000 rows,
     `results/open_leakage_pareto_audit_results.csv` with 12 rows,
     `results/statistical_audit_results.csv` with 139 rows, and
     `results/resource_audit_results.csv` with 26 rows.
...
4. Manuscript consistency
   ...
   - `results/open_leakage_continuation_sweep_summary.md` and
     ...
     `code/open_leakage_continuation_sweep.py`. The best continuation row
     reaches combined-noise mean fidelity `0.920133`, worst held-out fidelity
     `0.766810`, and mean maximum leakage `0.053478`; it improves the
     no-reference mean-fidelity/mean-leakage tradeoff but remains below the
     reference-assisted horizon and leakage-GRAPE terminal ceiling.
   - `results/open_leakage_pareto_audit_summary.md` and
     ...
     continuation point to classify the mean-fidelity/mean-maximum-leakage
     tradeoff. The audit shows that direct, Pareto-refined, integrated,
     target-biased, and continuation no-reference horizons provide useful
     Pareto tradeoff points, while the two-stage direct row improves the
     no-reference worst seed but is dominated in the two-dimensional Pareto
     audit by the continuation row, reference-assisted horizon, and
     leakage-GRAPE.
...
- Added `code/open_leakage_pareto_refinement.py`, generated the lower-to-middle
  no-reference open-leakage Pareto refinement rows, and regenerated the
  open-leakage Pareto audit with 11 tradeoff points.
