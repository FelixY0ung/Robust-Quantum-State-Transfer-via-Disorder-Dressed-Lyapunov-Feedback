...
1. Syntax/reproducibility checks
   ...
   - Confirmed output sizes:
    ...
    - `results/ensemble_lyapunov_results.csv`: 100 rows, Z/H tasks, seeds
     ...
     - `results/open_leakage_pareto_refinement_results.csv`: 1000 rows, 5
       controllers, 4 noise cases, strength `0.03`, seeds `10..59`.
     - `results/open_leakage_pareto_audit_results.csv`: 11 rows after adding
       the Pareto-refinement points.
   ...
   - Ran `python3 code/reproducibility_manifest.py`; the manifest recorded
     SHA-256 hashes, file sizes, CSV row counts, and CSV columns for 44 code
     files and 141 result artifacts. Key row-count checks include
     ...
     `results/open_leakage_pareto_audit_results.csv` with 11 rows,
     `results/statistical_audit_results.csv` with 139 rows, and
     `results/resource_audit_results.csv` with 25 rows.
...
4. Manuscript consistency
   ...
   - `results/open_leakage_pareto_audit_summary.md` and
     ...
     held-out rows, integrated sweep point, and Pareto-refinement points to classify the
     mean-fidelity/mean-maximum-leakage tradeoff. The audit shows that direct,
     Pareto-refined, integrated, and target-biased no-reference horizons provide
     useful Pareto tradeoff points, while the two-stage direct row improves the
