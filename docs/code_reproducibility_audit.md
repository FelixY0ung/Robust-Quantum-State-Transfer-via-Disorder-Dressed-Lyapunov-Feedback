...
1. Syntax/reproducibility checks
   ...
   - Confirmed output sizes:
    ...
    - `results/ensemble_lyapunov_results.csv`: 100 rows, Z/H tasks, seeds
     ...
     - `results/transmon_open_leakage_adjoint_results.csv`: 400 rows, 2
       controllers, 4 noise cases, strength `0.03`, seeds `10..59`.
...
4. Manuscript consistency
   ...
   - `results/transmon_open_leakage_adjoint_summary.md` is generated by
     ...
     GRAPE-free direct Lindblad leakage-adjoint horizon diagnostic. It lowers
     leakage strongly but gives only a modest fidelity gain over the path seed.
     The same script supports `--plot-only` and writes
     `results/figures/transmon_open_leakage_combined.*`, combining the direct
     adjoint row with the path, reference-assisted adjoint, and leakage-GRAPE
     stress-test rows.
