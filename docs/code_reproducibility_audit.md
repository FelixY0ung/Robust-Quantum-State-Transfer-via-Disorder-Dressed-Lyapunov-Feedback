...
4. Manuscript consistency
   ...
   - `results/open_leakage_pareto_audit_summary.md` and
     ...
     held-out rows to classify the mean-fidelity/mean-maximum-leakage tradeoff
     and show that the two-stage direct row improves the no-reference worst seed
     but is dominated in the two-dimensional Pareto audit by the reference-assisted
     horizon and leakage-GRAPE.
